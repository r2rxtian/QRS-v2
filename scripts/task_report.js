...
function openRemarksModal(button, subtitle) {
  ...
  document.getElementById("remarksModalBody").innerHTML = source
    ? source.innerHTML
    : "";
...
function zoomPhoto(img) {
  ...
  zoomedPhoto.innerHTML =
    '<button type="button" class="zoomed-photo-close" aria-label="Close"><i class="fas fa-times"></i></button><img src="' +
    img.src +
    '">';
  document.body.appendChild(zoomedPhoto);
  zoomedPhoto.querySelector(".zoomed-photo-close").onclick = function () {
    document.body.removeChild(zoomedPhoto);
  };
...
async function exportReportToPDF() {
  ...
  try {
    ...
    const rows = Array.from(
      table.tBodies[0].querySelectorAll("tr:not(.filter-hidden)"),
    );
    ...
    function extractRemarksSegments(td) {
      ...
      Array.from(blocks).forEach(function (block, i) {
        ...
        segments.push({
          text:
            block.querySelector(".remark-block-label").textContent.trim() + ":",
          bold: true,
        });
        Array.from(block.querySelectorAll(".remark-block-text")).forEach(
          function (el) {
    ...
    // Company identity block at the top-left of every page. The contact
    // details intentionally sit directly beside the logo, matching the
    // layout used on La Rose Noire's controlled paper forms. The logo is
    // loaded once up front (addImage needs an already-resolved data URL)
    // and then redrawn on each page via autoTable's didDrawPage hook below
    // -- previously this block only ran once before autoTable(), which is
    // why only page 1 ever got the header.
    ...
    try {
      logoDataUrl = (await loadImageForPdf(LOGO_PATH)).dataUrl;
    } catch (e) {
    ...
    function drawHeader() {
      if (logoDataUrl) {
        doc.addImage(logoDataUrl, "JPEG", 5, 5, LOGO_SIZE, LOGO_SIZE);
      }
      ...
      doc.setFontSize(7.5);
      doc.text(
        "Lot 1-A & B, Clark IE-05 Area, M.A. Roxas Highway,",
        COMPANY_TEXT_X,
        13.5,
      );
      doc.text("Clark Freeport Zone, Philippines", COMPANY_TEXT_X, 17);
      doc.text(
        "Tel: +63 45 499-3010   |   Fax: +63 45 499 2346",
        COMPANY_TEXT_X,
        21,
      );
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      doc.text("Generated: " + generatedAt, PAGE_RIGHT, 20, { align: "right" });
    }
    ...
    const PAGE_MARGIN = 5;
    const REMARKS_COL_WIDTH =
      doc.internal.pageSize.getWidth() -
      PAGE_MARGIN * 2 -
      OTHER_FIXED_COLS_WIDTH -
      IMAGES_COL_WIDTH;
    ...
    function buildRemarkLines(segments) {
      ...
      segments.forEach(function (seg) {
        ...
        doc
          .splitTextToSize(seg.text, REMARKS_TEXT_WIDTH)
    ...
    doc.autoTable({
      ...
      // margin.top reserves the same 34mm on every subsequent page (not
      // just the first, which startY alone would cover) so the header
      // drawn by didDrawPage below never gets overlapped by table rows.
      margin: { top: 34, left: 5, right: 5, bottom: 16 }, // keeps table rows clear of the document-control footer drawn below
      ...
      columnStyles: {
        ...
        2: { cellWidth: 20, halign: "center" }, // Scheduled Date/Time
        3: { cellWidth: 15, halign: "center" }, // Biometrics
        4: {
          cellWidth: 16,
          halign: "left",
          cellPadding: { top: 1, right: 1.5, bottom: 1, left: 1.5 },
        }, // User -- widened; Start/End Time need less width now that each stacks onto two shorter lines instead of one long one
        5: {
          cellWidth: 14,
          halign: "center",
        ...
        9: {
          cellWidth: IMAGES_COL_WIDTH,
          halign: "center",
          minCellHeight: IMAGES_ROW_HEIGHT,
        }, // Images -- dedicated space, same on every row
      },
      ...
      didDrawCell: function (data) {
        if (data.section === "body" && data.column.index === 7) {
          ...
          let y =
            data.cell.y +
            (data.cell.height - textBlockHeight) / 2 +
            LINE_HEIGHT * 0.8;
        ...
        const boxSize = Math.max(
          0,
          Math.min(THUMB_SIZE, data.cell.height - THUMB_PAD * 2),
        );
        ...
        const groupWidth =
          photos.length > 0
            ? photos.length * boxSize + (photos.length - 1) * THUMB_GAP
            : 0;
        const groupStartX =
          data.cell.x + Math.max(THUMB_PAD, (data.cell.width - groupWidth) / 2);
        photos.forEach(function (photo, i) {
          const boxX = groupStartX + i * (boxSize + THUMB_GAP);
          if (boxSize <= 0 || boxX + boxSize > data.cell.x + data.cell.width)
            return;
          const boxY =
            data.cell.y + Math.max(0, (data.cell.height - boxSize) / 2);
    ...
    for (let p = 1; p <= pageCount; p++) {
      ...
      doc.text(FOOTER_EFFECTIVITY, pageWidth - 5, pageHeight - 7, {
        align: "right",
      });
      doc.text(FOOTER_REVISION, pageWidth - 5, pageHeight - 3.5, {
        align: "right",
      });
    }
    doc.setTextColor(0, 0, 0);
...
function mountTaskReportTable() {
  ...
  const searchInput = document.querySelector(
    '.table-search-input[data-target="reportTable"]',
  );
  const dateFromInput = document.querySelector(
    '.filter-wrap[data-table="reportTable"] input[data-filter-date="from"]',
  );
  const dateToInput = document.querySelector(
    '.filter-wrap[data-table="reportTable"] input[data-filter-date="to"]',
  );
  if (
