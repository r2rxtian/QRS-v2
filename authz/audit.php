 ...
 */
function writeAuditLog(?int $userId, string $action, ?string $entityType, ?int $entityId, ?int $departmentId, array $details = []): void
{
    $stmt = db()->prepare('
        INSERT INTO ' . T_AUDIT_LOG . ' (user_id, action, entity_type, entity_id, department_id, details, ip_address, user_agent)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ');
